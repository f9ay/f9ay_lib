...
extern "C" {
    ...
    char *f9ay_read(const char *filename, int *row, int *col) {
        try{
            std::filesystem::path path(filename);
            ...
            std::visit(
                ...
                importer);
            std::tuple<char *, int, int> res;
            std::visit(
                [&res]<typename T>(T &&mtx) {
                    int r = mtx.row();
                    int c = mtx.col();
                    res = {reinterpret_cast<char *>(mtx.transfer_ownership()), r, c};
                },
                image_midway);
            *row = std::get<1>(res);
            *col = std::get<2>(res);
    ...
    int f9ay_png_export(const char *filename, const char *data, int row, int col, int channels) {
        ...
        return -1; // TODO: Implement PNG export
    }
